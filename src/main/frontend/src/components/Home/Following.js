...
const Following = ({ active, handleActive, handlePage, handleFollower }) => {
    const { isLoggedIn, follower, following, setFollowing } = useContext(UserContext);
    ...
    const delFollowing = async(e) => {
        const response = await FollowApi.delFollowing(e);
        if(response){
            setData(prevData => prevData.filter(item => item.id !== e));
                    ...
                    <div className="FollowingContainer">
                        {data.map((datas) => (
                            <div key={datas.id} className="userContainer">
                                ...
                                <p>{datas.nickname}</p>
                                <button onClick={()=>delFollowing(datas.id)}>UnFollowing</button>
                            </div>
                        ))}
                        {data.length === 0 && (
